...
class NotificationModel extends Model
{
    ...
    // IMPORTANTE: usar string vazia (não `false`) para desabilitar o campo de
    // "updated_at" — ver convenção do CodeIgniter\BaseModel::setUpdatedField(),
    // que checa `$this->updatedField !== ''`. Com `false`, a checagem passa
    // (false !== '' é true) e `array_key_exists(false, $row)` é avaliado como
    // `array_key_exists(0, $row)`, injetando uma chave inteira `0` no array de
    // dados do insert — o que quebra `BaseBuilder::setBind()` sob tipagem
    // estrita no CI 4.6.4 ("Argument #1 ($key) must be of type string, int
    // given") e faz `notify()` retornar `false` SEMPRE, silenciosamente.
    // Constatado em produção em 07/06/2026: a tabela `notifications` tinha
    // ZERO linhas desde sempre — todo `notify()` do sistema falhava.
    protected $updatedField  = ''; // Notifications are immutable
