...
/**
 * Safe Session handler using file system for storage
 *
 * This handler extends BaseHandler directly to avoid ini_set() calls
 * that may be disabled in shared hosting environments.
 */
class SafeFileHandler extends BaseHandler
{
    /**
    ...
    public function __construct(SessionConfig $config, string $ipAddress)
    {
        ...
        // Set save path without using ini_set()
        if (!empty($this->savePath)) {
            $this->savePath = rtrim($this->savePath, '/\\');
            // Don't call ini_set() - just use the configured path
        } else {
            // Try to get session path from ini, but don't fail if we can't
            try {
        ...
        // Configure session ID regex without using ini_set()
     ...
     *
     * This version doesn't use ini_set() to avoid errors in restricted environments.
     */
    protected function configureSessionIDRegex()
    {
        // Try to get current values, but don't fail if ini_get is restricted
        try {
            $bitsPerCharacter = (int) @ini_get('session.sid_bits_per_character');
            $sidLength        = (int) @ini_get('session.sid_length');
        } catch (\Throwable $e) {
            // Use PHP defaults if we can't read ini settings
            $bitsPerCharacter = 4;
            $sidLength        = 32;
        }

        // Default to PHP 9 standards (4 bits, 32 length)
        ...
        // Always use the safe default regex pattern
