                                    ...
                                    <td class="text-end">
                                        <div class="table-icon-actions">
                                            ...
                                            <button type="button"
                                                    ...
                                                    title="Excluir"
                                                    onclick="confirmDeleteDepartment(<?= $deptId ?>, '<?= esc(addslashes($dept->name ?? ''), 'js') ?>')">
    ...
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <p>Tem certeza que deseja excluir o departamento <strong id="deleteDepartmentName"></strong>?</p>
                <p class="text-muted small mb-0">
                    <i class="bi bi-info-circle me-1"></i>
                    Esta ação não pode ser desfeita. Colaboradores e cargos vinculados a este departamento precisarão ser reatribuídos ou removidos antes da exclusão.
                </p>
            </div>
            <div class="modal-footer border-0 pt-0">
                ...
                <form id="deleteDepartmentForm" method="post" class="d-inline">
                    <?= csrf_field() ?>
                    <button type="submit" class="btn btn-danger" id="deleteDepartmentBtn">
...
function confirmDeleteDepartment(id, name) {
    ...
    document.getElementById('deleteDepartmentForm').action = '<?= site_url('settings/departments/') ?>' + id + '/delete';

    const btn = document.getElementById('deleteDepartmentBtn');
    btn.disabled = false;
    btn.innerHTML = '<i class="bi bi-trash-fill me-2"></i>Excluir';

    document.getElementById('deleteDepartmentForm').onsubmit = async function(e) {
        e.preventDefault();
        btn.disabled = true;
