...
<?= $this->section('title') ?>Tipos de Contrato<?= $this->endSection() ?>
...
<div class="container-fluid sp-module-stack">
    ...
    <?= view('components/page_header', [
        ...
        'subtitle' => 'Gerencie os tipos de contrato disponíveis no cadastro de colaboradores.',
        'icon'     => 'bi bi-file-earmark-text-fill',
        'actions'  => [
            ['label' => 'Novo Tipo de Contrato', 'icon' => 'bi bi-plus-lg', 'url' => route_to('settings.contract-types.create')],
        ],
    ]) ?>
                    ...
                    <tbody>
                        ...
                        <?php else: ?>
                            <?php foreach ($contractTypes as $type):
                                ...
                                $isActive = ($type->active === true || $type->active === 't');
                            ?>
                                <tr id="row-contract-type-<?= $typeId ?>">
                                    <td><strong><?= esc($type->name ?? '') ?></strong></td>
                                    <td class="text-muted"><?= esc($type->description ?? '-') ?></td>
                                    ...
                                    <td class="text-end">
                                        <div class="table-icon-actions">
                                            <a href="<?= sp_route_url('settings.contract-types.edit', $typeId) ?>"
                                            ...
                                            <button type="button"
                                                    ...
                                                    onclick="catalogToggle(this, '<?= sp_route_url('settings.contract-types.toggle', $typeId) ?>')">
                                                <i class="bi <?= $isActive ? 'bi-toggle-on' : 'bi-toggle-off' ?>"></i>
                                            </button>
                                            <button type="button"
                                                    class="icon-action icon-action-danger"
                                                    title="Excluir"
                                                    onclick="confirmDeleteContractType(<?= $typeId ?>, '<?= esc(addslashes($type->name ?? ''), 'js') ?>')">
...
<div class="modal fade" id="deleteContractTypeModal" tabindex="-1" aria-labelledby="deleteContractTypeModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            <div class="modal-header border-0 pb-0">
                <h5 class="modal-title text-danger" id="deleteContractTypeModalLabel">
                    <i class="bi bi-exclamation-triangle-fill me-2"></i>Confirmar exclusão
                </h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
            </div>
            <div class="modal-body">
                <p>Tem certeza que deseja excluir o tipo de contrato <strong id="deleteContractTypeName"></strong>?</p>
                <p class="text-muted small mb-0">
                    <i class="bi bi-info-circle me-1"></i>
                    Esta ação não pode ser desfeita. Colaboradores ativos vinculados a este tipo de contrato precisarão ser reatribuídos antes da exclusão.
                </p>
            </div>
            <div class="modal-footer border-0 pt-0">
                <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Cancelar</button>
                <form id="deleteContractTypeForm" method="post" class="d-inline">
                    <?= csrf_field() ?>
                    <button type="submit" class="btn btn-danger" id="deleteContractTypeBtn">
...
function confirmDeleteContractType(id, name) {
    document.getElementById('deleteContractTypeName').textContent = name;
    document.getElementById('deleteContractTypeForm').action = '<?= site_url('settings/contract-types/') ?>' + id + '/delete';

    const btn = document.getElementById('deleteContractTypeBtn');
    btn.disabled = false;
    btn.innerHTML = '<i class="bi bi-trash-fill me-2"></i>Excluir';

    document.getElementById('deleteContractTypeForm').onsubmit = async function(e) {
        e.preventDefault();
        btn.disabled = true;
        ...
        try {
            ...
            const modal = bootstrap.Modal.getInstance(document.getElementById('deleteContractTypeModal'));
            modal.hide();
            if (json.success) {
                location.reload();
            } else {
    ...
    new bootstrap.Modal(document.getElementById('deleteContractTypeModal')).show();
}
</script>
<?= $this->endSection() ?>
