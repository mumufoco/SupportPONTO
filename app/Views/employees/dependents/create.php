...
<?= $this->section('title') ?>Novo dependente<?= $this->endSection() ?>
...
<div class="container-fluid sp-responsive-screen">
    <?= view('components/page_header', [
        ...
        'subtitle' => 'Cadastre um dependente vinculado a um colaborador ativo.',
    ...
    <form action="<?= site_url('employees/dependents') ?>" method="post">
            ...
            <div class="sp-form-card__body">
                ...
                <select name="employee_id" id="employee_id" class="form-select" required>
                    ...
                    <?php foreach ($employees ?? [] as $emp): ?>
                        <option value="<?= esc($emp->id) ?>" <?= (int) ($preselectedEmployeeId ?? 0) === (int) $emp->id ? 'selected' : '' ?>>
                ...
                <div class="row g-3">
                    <div class="col-12 col-md-4">
                        <div class="form-check form-switch">
                            <input class="form-check-input" type="checkbox" name="irrf_dependent" id="irrf_dependent" value="1" checked>
                            <label class="form-check-label" for="irrf_dependent">Conta para IRRF</label>
                        </div>
                    </div>
                    <div class="col-12 col-md-4">
                        <div class="form-check form-switch">
                            <input class="form-check-input" type="checkbox" name="family_allowance_dependent" id="family_allowance_dependent" value="1">
                            <label class="form-check-label" for="family_allowance_dependent">Conta para salário-família</label>
                        </div>
                    </div>
                    <div class="col-12 col-md-4">
                        <div class="form-check form-switch">
                            <input class="form-check-input" type="checkbox" name="has_disability" id="has_disability" value="1">
                            <label class="form-check-label" for="has_disability">Possui deficiência</label>
                        </div>
                    </div>
                    <div class="col-12">
                        <label for="notes" class="form-label">Observações</label>
                        <textarea name="notes" id="notes" class="form-control" rows="3" placeholder="Ex: documentação comprobatória, condições especiais, etc."></textarea>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="sp-form-actions">
            ...
            <button type="submit" class="btn btn-primary btn-lg px-4">
                <i class="bi bi-save-fill me-1"></i> Cadastrar dependente
            </button>
        </div>
    </form>
</div>
...
SupportPontoValidation.bindCpfField(document.getElementById('cpf'));
</script>
<?= $this->endSection() ?>
