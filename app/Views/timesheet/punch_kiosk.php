...
<?= $this->section('title') ?>Terminal de Ponto - SupportPONTO<?= $this->endSection() ?>
...
$labels = [
    ...
    'cpf'       => ['icon' => 'bi-card-text',           'title' => 'CPF',      'desc' => 'Informe o CPF'],
    'qrcode'    => ['icon' => 'bi-qr-code-scan',        'title' => 'QR Code',  'desc' => 'Escaneie o crachá'],
    'facial'    => ['icon' => 'bi-person-bounding-box', 'title' => 'Facial',   'desc' => 'Reconhecimento facial'],
    'biometria' => ['icon' => 'bi-fingerprint',         'title' => 'Biometria','desc' => 'Impressão digital'],
];
?>
...
<div class="container-fluid px-3 py-3" style="max-width:900px;margin:0 auto;">
    ...
    <!-- Resultado -->
    <div id="kioskPunchResult" class="mb-3"></div>
    ...
    <div class="sp-vstepper">
                    ...
                    <div class="card-body py-3">
                        <div class="d-flex flex-wrap gap-2" id="punchTypeGroup">
                            <?php foreach ([
                                ...
                                'intervalo_inicio' => ['label' => 'Inicio Intervalo', 'icon' => 'bi-pause-circle',       'color' => 'warning'],
                                'intervalo_fim'    => ['label' => 'Fim Intervalo',    'icon' => 'bi-play-circle',        'color' => 'info'],
                                'saida'            => ['label' => 'Saida',            'icon' => 'bi-box-arrow-right',    'color' => 'danger'],
                            ] as $value => $item): ?>
                                <button type="button"
                ...
                <div class="card border-0 shadow-sm">
                    <div class="card-body py-3">
                        <div class="row g-2" id="methodSelector">
                            ...
                            <div class="col">
                                <button type="button"
                                        class="btn w-100 h-100 py-3 method-btn border d-flex flex-column align-items-center gap-1 <?= $enabled ? 'btn-outline-primary' : 'btn-outline-secondary opacity-50' ?>"
                                        data-method="<?= $key ?>"
                                        <?= !$enabled ? 'disabled aria-disabled="true"' : '' ?>
                                        title="<?= $enabled ? esc($info['desc']) : 'Metodo desabilitado.' ?>">
        ...
        <div class="sp-vstepper__step" id="qs3">
            ...
            <div class="sp-vstepper__body">
                <div class="sp-vstepper__title d-flex justify-content-between align-items-center">
                    <span><i class="bi bi-check2-circle"></i> Identificacao</span>
                    <button type="button" class="btn btn-outline-secondary btn-sm" id="resetKioskFlow">
                ...
                <div class="card border-0 shadow-sm">
                    <div class="card-body">
                        ...
                        <form id="form-codigo" class="method-form" data-method="codigo">
                            <label class="form-label fw-semibold"><i class="bi bi-person-badge me-1 text-primary"></i>Codigo do colaborador</label>
                            <div class="input-group">
                                <input type="text" class="form-control form-control-lg" name="unique_code" placeholder="Ex.: COL001" autofocus autocomplete="off">
                                <button class="btn btn-success px-4" type="submit"><i class="bi bi-check-lg me-1"></i>Registrar</button>
                            </div>
                        </form>

                        <form id="form-cpf" class="method-form" style="display:none;" data-method="cpf">
                            <label class="form-label fw-semibold"><i class="bi bi-card-text me-1 text-primary"></i>CPF</label>
                            <div class="input-group">
                                <input type="text" class="form-control form-control-lg" id="cpf" name="cpf" placeholder="000.000.000-00" inputmode="numeric" autocomplete="off">
                                <button class="btn btn-success px-4" type="submit"><i class="bi bi-check-lg me-1"></i>Registrar</button>
                            </div>
                        </form>

                        <div id="form-qrcode" class="method-form" style="display:none;" data-method="qrcode">
                            <label class="form-label fw-semibold"><i class="bi bi-qr-code-scan me-1 text-primary"></i>QR Code do cracha</label>
                            <p class="text-muted small mb-2">Posicione o QR Code na frente da camera.</p>
                            <div id="qr-reader" class="sp-quick-reader rounded border"></div>
                        </div>

                        <div id="form-facial" class="method-form" style="display:none;" data-method="facial">
                            <label class="form-label fw-semibold"><i class="bi bi-person-bounding-box me-1 text-primary"></i>Reconhecimento Facial</label>
                            <div id="faceCameraStatus" class="alert alert-secondary mb-2" style="font-size:.82rem">
                                <div class="d-flex align-items-center gap-2">
                                    ...
                                    <span>Iniciando camera...</span>
                                </div>
                            </div>
                            <div class="text-center">
                                <div style="position:relative;display:inline-block;width:100%;max-width:480px">
                                    <video id="faceVideo" autoplay playsinline class="sp-quick-video rounded border mb-2 w-100" style="max-width:100%;background:#111;min-height:160px;display:block"></video>
                                    <svg style="position:absolute;top:0;left:0;width:100%;height:100%;pointer-events:none;border-radius:.375rem">
                                        <defs><mask id="pkfMask"><rect width="100%" height="100%" fill="white"/><ellipse cx="50%" cy="44%" rx="27%" ry="37%" fill="black"/></mask></defs>
                                        <rect width="100%" height="100%" fill="rgba(0,0,0,0.38)" mask="url(#pkfMask)"/>
                        ...
                        <div id="form-biometria" class="method-form" style="display:none;" data-method="biometria">
                            <label class="form-label fw-semibold"><i class="bi bi-fingerprint me-1 text-primary"></i>Biometria Digital</label>
                            <div id="bioMessage" class="alert alert-secondary mb-3" style="font-size:.82rem">
                                <div class="d-flex align-items-center gap-2">
                                    <div class="spinner-border spinner-border-sm flex-shrink-0" role="status"></div>
                                    <span>Verificando suporte biometrico...</span>
                                </div>
                            </div>
...
(() => {
    ...
    let qrScanner = null, faceStream = null, fpWs = null, resetTimer = null;
    ...
    const resultDiv      = document.getElementById('kioskPunchResult');
    ...
    document.getElementById('resetKioskFlow').addEventListener('click', resetTerminal);

    function scheduleAutoReset() {
        clearTimeout(resetTimer);
        resetTimer = setTimeout(resetTerminal, 8000);
    }
    ...
    function resetTerminal() {
        ...
        punchTypeInput.value = 'entrada';
        document.querySelectorAll('.punch-type-btn').forEach(b => {
            b.classList.remove('active', 'btn-' + b.dataset.color);
            b.classList.add('btn-outline-' + b.dataset.color);
            if (b.dataset.value === 'entrada') {
                b.classList.remove('btn-outline-' + b.dataset.color);
                b.classList.add('btn-' + b.dataset.color, 'active');
            }
        });
        document.querySelectorAll('.method-btn').forEach(b => {
            if (!b.disabled) { b.classList.remove('btn-primary'); b.classList.add('btn-outline-primary'); }
        });
        ['form-codigo','form-cpf'].forEach(id => { var f = document.getElementById(id); if (f && f.reset) f.reset(); });
        resultDiv.innerHTML = '';
        qsDone = {1: false, 2: false};
        markQS(1, false); markQS(2, false);
        document.getElementById('qs3').classList.remove('is-active','is-done');
        activeMethod = enabledMethods.codigo ? 'codigo' : (Object.keys(enabledMethods).find(k => enabledMethods[k]) || 'codigo');
    }
    ...
    async function submit(endpoint, payload) {
        clearTimeout(resetTimer);
        try {
            var response = await spFetch(endpoint, {
                method: 'POST',
            ...
            var ok = response.ok;
            var extra = '';
            var pending = result && result.data && result.data.pending_punch ? result.data.pending_punch : null;
            if (pending && pending.eligible && pending.justify_url) {
                var raw = String(pending.justify_url);
                var safe = /^(https?:\/\/|\/)/.test(raw) ? raw : '#';
                extra = '<div class="mt-2"><a class="btn btn-sm btn-warning" href="' + safe + '">Abrir justificativa</a></div>';
            }
            resultDiv.innerHTML = '<div class="alert ' + (ok ? 'alert-success' : 'alert-danger') + ' d-flex align-items-start gap-2">'
                ...
                + '<div>' + (result.message || (ok ? 'Registro efetuado.' : 'Falha ao registrar.')) + extra + '</div></div>';
            if (ok) scheduleAutoReset();
        } catch (_) {
            resultDiv.innerHTML = '<div class="alert alert-danger"><i class="bi bi-wifi-off me-2"></i>Falha de comunicacao com o servidor.</div>';
        }
    }
    ...
    document.getElementById('form-cpf').addEventListener('submit', async e => {
        e.preventDefault();
        await submit(endpoints.cpf, {cpf: (cpfEl ? cpfEl.value : '').trim(), punch_type: punchTypeInput.value});
    });

    async function startFace() {
        var status = document.getElementById('faceCameraStatus');
        if (!supportsCamera) {
            if (status) { status.className = 'alert alert-warning mb-2'; status.innerHTML = '<i class="bi bi-camera-video-off me-2"></i>Camera indisponivel.'; }
            return;
        }
        try {
            ...
            if (status) { status.className = 'alert alert-success mb-2'; status.innerHTML = '<i class="bi bi-camera-video-fill me-2"></i>Camera pronta. Posicione o rosto.'; }
        } catch(e) {
            if (status) { status.className = 'alert alert-danger mb-2'; status.innerHTML = '<i class="bi bi-x-circle-fill me-2"></i>Nao foi possivel acessar a camera.'; }
        }
    }
    ...
    document.getElementById('captureFace').addEventListener('click', async () => {
        ...
        if (!faceStream || !(video.videoWidth > 0)) {
            resultDiv.innerHTML = '<div class="alert alert-warning"><i class="bi bi-camera-video-off me-2"></i>Camera ainda nao esta pronta.</div>';
            return;
        }
    ...
    async function startQR() {
        if (!supportsCamera || typeof Html5Qrcode === 'undefined') {
            resultDiv.innerHTML = '<div class="alert alert-warning">Leitor QR Code indisponivel.</div>';
            return;
        }
        try {
            ...
            resultDiv.innerHTML = '<div class="alert alert-warning">Nao foi possivel iniciar o leitor QR Code.</div>';
        }
    }
